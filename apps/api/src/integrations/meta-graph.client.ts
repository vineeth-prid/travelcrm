...
/**
 * Thin HTTP client for the Meta Graph API, shared by WhatsApp and Instagram.
 * Its only real job is turning Meta's error envelope into something a
 * salesperson can act on.
 *
 * The default host is graph.facebook.com. Instagram messaging lives on
 * graph.instagram.com and passes its own `baseUrl` — the two are different
 * APIs and calling the wrong host is the classic way to break this integration.
 */
@Injectable()
export class MetaGraphClient {
  ...
  async get<T>(channel: string, path: string, accessToken: string, baseUrl?: string): Promise<T> {
    return this.send<T>(channel, path, accessToken, { method: 'GET' }, baseUrl);
  }
  ...
  private async send<T>(
    ...
    try {
      response = await fetch(`${baseUrl ?? this.baseUrl}${path}`, {
        ...init,
